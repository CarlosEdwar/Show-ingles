import { User, Question, Reward, GameState } from '@/types';
...
export function registerUser(email: string, password: string, name: string): { success: boolean; message: string } {
  if (typeof window === 'undefined') return { success: false, message: 'Erro no servidor' };
...
export function addReward(reward: Reward) {
  ...
  const rewards: Reward[] = JSON.parse(localStorage.getItem(STORAGE_KEYS.REWARDS) || '[]');
  rewards.push(reward);
...
export function completeQuestion(questionId: string, points: number) {
  ...
  if (!user.completedQuestions?.includes(questionId)) {
    user.points += points;
    user.completedQuestions = [...(user.completedQuestions || []), questionId];
    
    // Verificar progresso de nível
    const questionsPerLevel = 2;
    const completedAtLevel = user.completedQuestions.filter(q => {
      const question = getQuestions().find(qt => qt.id === q);
